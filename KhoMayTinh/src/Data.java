...
public class Data {
    ...
    public static void Tieude(int T) {
        switch (T) {
            ...
            case 7:
                ...
                break;
            case 8:
                System.out.print("So Luong Con Lai: ");
                break;
            case 9:
    ...
    public static void TieuDeMTB(int T) {
        switch (T) {
            ...
            case 9:
                ...
                break;
            case 10:
                System.out.print("So Luon Xuat: ");
                break;
            case 11:
                System.out.print("Thoi Gian Xuat: ");
                break;
            case 12:
                System.out.print("So Luong Con Loai: ");
                break;
        }
    }
    ...
    public static void CapNhat(String MaSql, String Ten, String MaSP, String Cot, String ThayDoi, int Ngay, int Thang,
            int Nam) {
        Data Dt = new Data();
        if (Ten != null) {
            if (Cot == "STT" || Cot == "SO-LUONG" || Cot == "SOLUONG-NHAP" || Cot == "SOLUONGBANTRONG-NGAY") {
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate(
                            "UPDATE " + MaSql + " SET [" + Cot + "] = " + ThayDoi + " WHERE [TEN] = '" + Ten + "';");
                    System.out.println("\nCap Nhat Thanh Cong!\n");
                ...
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate("UPDATE " + MaSql + " SET [" + Cot + "] = convert(datetime,'" + Ngay + "-"
                            + Thang + "-" + Nam + "',103) WHERE [TEN] = '" + Ten + "';");
                    System.out.println("\nCap Nhat Thanh Cong!\n");
                    TimKiem(MaSql, 12, 1, Ten, null);
                } catch (SQLException ex) {
                    System.out.println("Cap Nhat Khong Thanh Cong!");
            ...
            if (Cot == "STT" || Cot == "SO-LUONG" || Cot == "SOLUONG-NHAP" || Cot == "SOLUONGBANTRONG-NGAY") {
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate(
                            "UPDATE " + MaSql + " SET [" + Cot + "] = " + ThayDoi + " WHERE [MA-SP] = '" + MaSP + "';");
                    System.out.println("\nCap Nhat Thanh Cong!\n");
                    TimKiem(MaSql, 12, 1, Ten, null);
                } catch (SQLException ex) {
                    System.out.println("Cap Nhat Khong Thanh Cong!");
                ...
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate("UPDATE " + MaSql + " SET [" + Cot + "] = convert(datetime,'" + Ngay + "-"
                            + Thang + "-" + Nam + "',103) WHERE [MA-SP] = '" + MaSP + "';");
                    System.out.println("\nCap Nhat Thanh Cong!\n");
                    TimKiem(MaSql, 12, 1, Ten, null);
                } catch (SQLException ex) {
                    System.out.println("Cap Nhat Khong Thanh Cong!");
                ...
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate("UPDATE " + MaSql + " SET [" + Cot + "] = '" + ThayDoi + "' WHERE [MA-SP] = '"
                            + MaSP + "';");
                    System.out.println("\nCap Nhat Thanh Cong!\n");
                    TimKiem(MaSql, 12, 1, Ten, null);
                } catch (SQLException ex) {
    ...
    public static void ThemPhuKienPC(String MaSql, String MAPK, String TEN, String NSX, int SOLUONGNHAP, int NGAYNHAP,
            int THANGNHAP, int NAMNHAP, int SOLUONGXUAT, int NGAYXUAT, int THANGXUAT, int NAMXUAT) {
        Scanner s = new Scanner(System.in);
        Data Dt = new Data();
        try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
    ...
    public static void CapNhatPhuKienPC(String MaSql, String MaPK, String Ten, String Cot, String ThayDoi, int Ngay,
            int Thang, int Nam) {
        Data Dt = new Data();
        if (Ngay != 0 || Thang != 0 || Nam != 0) {
            try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                stat.executeUpdate("UPDATE " + MaSql + " SET [" + Cot + "] = convert(datetime,'" + Ngay + "-" + Thang
                        + "-" + Nam + "',103) WHERE [TEN] = '" + Ten + "';");
                System.out.println("\nCap Nhat Thanh Cong!\n");
            ...
            try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                stat.executeUpdate(
                        "UPDATE " + MaSql + " SET [" + Cot + "] = '" + ThayDoi + "' WHERE [Ma] = '" + MaPK + "';");
                System.out.println("\nCap Nhat Thanh Cong!\n");
                TimKiemPhuKienPC(MaSql, MaPK, Ten);
            } catch (SQLException ex) {
                System.out.println("Cap Nhat Khong Thanh Cong!");
    ...
    public static void TimkiemMTB(String MaSql, String MaPK, String Ten) {
        if (MaPK != null) {
            Data Dt = new Data();
            ResultSet resultSet = null;
            try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
    ...
    public static void CapNhatMTB(String MaSql, String TenCot, String GiaTri, String MaPK, String Ten, int Ngay,
            int Thang, int Nam) {
        if (MaPK != null) {
            if (Ngay != 0 || Thang != 0 || Nam != 0) {
                Data Dt = new Data();
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate("UPDATE " + MaSql + " SET [" + TenCot + "] = convert(datetime,'" + Ngay + "-"
                            + Thang + "-" + Nam + "',103) WHERE [Ma] = '" + MaPK + "'");
                    System.out.println("Cap Nhat Thanh Cong!");
                    TimkiemMTB(MaSql, MaPK, Ten);
                } catch (SQLException ex) {
                    System.out.println("Khong co du lieu!");
                }
            } else {
                ...
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate(
                            "UPDATE " + MaSql + " SET [" + TenCot + "] = '" + GiaTri + "' WHERE [Ma] = '" + MaPK + "'");
                    System.out.println("Cap Nhat Thanh Cong!");
                    TimkiemMTB(MaSql, MaPK, Ten);
                } catch (SQLException ex) {
                    System.out.println("Khong co du lieu!");
            ...
            if (Ngay != 0 || Thang != 0 || Nam != 0) {
                ...
                try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
                    stat.executeUpdate(
                            "UPDATE " + MaSql + " SET [" + TenCot + "] = '" + GiaTri + "' WHERE [Ten] = '" + Ten + "'");
                    System.out.println("Cap Nhat Thanh Cong!");
                    TimkiemMTB(MaSql, MaPK, Ten);
                } catch (SQLException ex) {
                    System.out.println("Khong co du lieu!");
    ...
    public static void ThemMTB(String MaSql, String Ma, String Ten,String Hang,String ManHinh, String Cpu, String Ram, String Rom,int SoluongN, int NgayN,int ThangN,int NamN, int SoLuongX,int NgayX,int ThangX,int NamX){
        Scanner s = new Scanner(System.in);
        Data Dt = new Data();
        try (Connection conn = DriverManager.getConnection(Dt.URL()); Statement stat = conn.createStatement()) {
            String Them = " INSERT INTO " + MaSql + " VALUES(N'"
                    ...
                    + "', N'" + Rom
                    + "'," + SoluongN
                    + ",convert(datetime,'" + NgayN + "-" + ThangN + "-" + NamN + "',103), "
                    + SoLuongX
                    + ", convert(datetime,'" + NgayX + "-" + ThangX + "-" + NamX + "',103));";
            stat.executeUpdate(Them);
            System.out.println("\nThem Thanh Cong!\n");
            MayTinhBang.KhoConMTB(MaSql);
        } catch (SQLException ex) {
            System.out.println("Them Khong Thanh Cong!\nThem Lai? (Y Hoac N): ");
            String YorN = s.nextLine();
            if (YorN.equals("Y") || YorN.equals("Y")) {
                MayTinhBang.ThemMTB(MaSql);
            } else {
                MayTinhBang.Menu();
            }
        }
